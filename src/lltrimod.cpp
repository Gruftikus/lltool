...
int llTriMod::Init(void) {
	...
	if (!triangles) {
		_llLogger()->WriteNextLine(-LOG_FATAL, "%s: no triangle list in map [%s]", command_name, map);
		return 0;
	}
	if (!triangles->GetN()) {
		_llLogger()->WriteNextLine(-LOG_FATAL, "%s: map [%s] not triangulated. Call MakeTriangulation before.", command_name, map);
		return 0;
	}
